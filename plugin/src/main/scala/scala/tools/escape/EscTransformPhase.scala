...
/*
    ...
    Anything inbetween in possible
...
*/
    ...
    def traverse(tree: Tree, m: Type, boundary: List[Symbol]): Unit = tree match {
      ...
      case Apply(fun, args) =>
        ...
        println(fun.symbol.name)
        println(fun.symbol.owner.name)
        println(fun.symbol.throwsAnnotations.map(_.tpe))
